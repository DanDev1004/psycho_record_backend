...
export const obtenerTodos = async (req, res) => {
    try {
        ...
        if (req.session.role === 1) {
            ...
            response = await Derivacion.findAll({
                attributes: ['ID_DERIVACION', 'ID_USUARIO', 'ID_LISTADO_AULA', 'MOTIVO', 'SEVERIDAD', 'ESTADO'],
                where: {
...
export const buscar = async (req, res) => {
    try {
        ...
        const response = await Derivacion.findAll({
            ...
            include: [
                ...
                },
                {
                    ...
                    include: [
                        {
                            model: Alumno,
                            attributes: ['DNI_ALUMNO', 'NOMBRE_ALUMNO', 'APELLIDO_ALUMNO'],
                        }
                    ],
                }
            ],
            where: {
                [Op.or]: [
                    { '$USUARIO.NOMBRE_USUARIO$': { [Op.like]: `%${searchText}%` } },
                    { '$USUARIO.APELLIDO_USUARIO$': { [Op.like]: `%${searchText}%` } },
...
export const crear = async (req, res) => {
    try {
        if (req.session.role === 1) {
            const { ID_USUARIO, ID_LISTADO_AULA, MOTIVO, SEVERIDAD } = req.body;
            ...
            const { ID_LISTADO_AULA, MOTIVO, SEVERIDAD } = req.body;
            await Derivacion.create({
                ID_USUARIO: req.session.userId,
                ID_LISTADO_AULA: ID_LISTADO_AULA,
                MOTIVO: MOTIVO,
...
export const actualizar = async (req, res) => {
    try {
        ...
        const { ID_LISTADO_AULA, MOTIVO, SEVERIDAD, ESTADO } = req.body;
